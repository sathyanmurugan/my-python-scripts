...
import salesforce.xmltramp2 as xmltramp
from salesforce.xmltramp2 import islst
...
class Client:
	def __init__(self):
		self.batchSize = 500
		self.serverUrl = "https://login.salesforce.com/services/Soap/u/38.0"
		self.__conn = None
		self.timeout = 15
...
class IterClient(Client,object):
    ...
    def chunkRequests(self, collection, chunkLength=None):
        if not islst(collection):
            ...
            for i in range(0, len(collection), chunkLength):
...
class BeatBoxXmlGenerator(XMLGenerator):
	...
	def startElementNS(self, name, qname, attrs):
		self._write(str('<' + self.makeName(name)))

		for pair in self._undeclared_ns_maps:
			self._write(str(' xmlns:%s="%s"' % pair))
		self._undeclared_ns_maps = []

		for (name, value) in list(attrs.items()):
			self._write(str(' %s=%s' % (self.makeName(name), quoteattr(value))))
		self._write(str('>'))
...
class SoapEnvelope:
	...
	def post(self, conn=None, alwaysReturnList=False):
		...
		if response.getheader('content-encoding','') == 'gzip':
			rawResponse = gzip.GzipFile(fileobj=BytesIO(response.read())).read().decode('utf-8')
		else:
